...
- [] day: int between 1 and 8. Other inputs should terminate the program and print f"The input number must be within the range 1-8. {user_input} was given.". Read the input with input("Enter day (1-8): "). Days 1 to 5 means a weekday, days 6 and 7 mean a weekend, and day 8 means a holiday.
- [] age: int between 0 and 80, or -1. The program will continuously ask for user input and assign it to int, until -1 was inputted. If the user inputs an invalid value, print f"The input number must be within the range 0-80, or -1. {user_input} was given.", ignore the invalid input, and ask for the next input. Read the input with input("Enter age (-1 to stop): ") 
...
4. Check if the day is on a weekend or holiday and least one complete group was formed. If yes, assign 0.1 to discount. 
